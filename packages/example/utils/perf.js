...
import { PERF_MAX, PERF_LIKE_MAX, PERF_AUTO, PERF_USING_COMPONENTS } from "./config";
...
class Perf {
  ...
  measure(endName) {
    ...
    performance.measure(endMarkName, this.name, endMarkName);
  }
...
function getStat(perf, type) {
  const before = perf
    .getEntriesByName(`${type}.setData.before`, "measure")
    .map((entry) => entry.duration);
  const end = perf
    .getEntriesByName(`${type}.setData.end`, "measure")
    .map((entry) => entry.duration);
...
function initPagePerf() {
  ...
  wrapperSetData.call(
    ...
    function (stat) {
      ...
      return `共${PERF_MAX}页,${20 * PERF_MAX}条数据,赋值平均耗时:${stat.before},渲染平均耗时:${
        stat.end
      },点击任意点赞按钮,开始按钮点击测试!`;
    },
    function () {
